...
if (isset($_POST['submit'])) {
  ...
  if ( isset( $_FILES['profile_img']) AND $_FILES['profile_img']['size'] > 0 ) {
  	...
  	if ( !empty($user['img']) AND $user['img'] != 1 ) {
  		remove_attachment( $user['img'] );
  	}
  	//update user profile image
  	$profile_img_id = add_attachment( 'profile_img' );
...
<h2 class="page-title mt-5">User Activity</h2>
<?php
	$user_id = (int) $_SESSION['id'];
	$stmt = $db->prepare("SELECT * FROM activity WHERE user_id = '{$user_id}'");
	$stmt->execute();
	while( $activity = $stmt->fetch(PDO::FETCH_ASSOC) ){
		echo '<p class="text-muted">' . $activity['title'] . '</p>';
	}
?>
